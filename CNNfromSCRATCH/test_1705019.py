...
def load_test(given_path):
    ...
    image_name=df['filename'].values
    labelcol=df['digit'].values
    # image_path=os.path.join(path_testing_folder, image_name)
    ...
    for i in range(len(image_name)):
        # print(i)
        # image_path=os.path.join(path_testing_folder, image_name[i])
        image_path = os.path.join(given_path, image_name[i])
        label=labelcol[i]
        temp_name= image_name[i]
    ...
    return x_test, np.array(labels), np.array(names)
...
x_test,y_test,test_names=load_test(given_path)
...
acc=accuracy_metric(y_test,predicted_label)

# loss1=cross_entropy_loss(y_validate,predicted_label)/32

y_t_one_hot_label = np.zeros((len(y_test), 10))

# set the corresponding position to 1 for each category
y_t_one_hot_label[np.arange(len(y_test)), y_test] = 1

loss2=crossEntropyLoss(y_t_one_hot_label,y_pred)/len(y_test)
# loss3=sklearn.metrics.log_loss(y_val_one_hot_label,y_pred)
f1=f1_score_metric(y_test,predicted_label)
